...
I’ve recently had to do a quick bit of DB performance analysis work on a 3rd Party database to see if I can improve performance(without being too invasive).  The quickest way to do this, is using the [Database Tuning Engine Advisor](http://bit.ly/cf52sU) tool that comes with SQL 2008 (the **_best_** way is to have loads of experience and a tonne of time to properly analyse the database).
...
Fortunately, the DTA created indexes and statistics are all prefixed with \***\*dta\*\***so it’s relatively easy to write a query to remove the automatically created scripts:
