...
description: 'Why catch-and-log exception handling is just as harmful as swallowing exceptions, and why throwing should be the default over logging.'
pubDate: 2011-05-03
tags: ['best practise']
source: blogger
originalUrl: 'http://martinondotnet.blogspot.com/2011/05/logging-is-new-exception-swallowing.html'
...
#### Every 'catch' block must (at a minimum) _throw_ or log
...
My response to this is simple...**_remove all exception handling_**!

In the vast majority of cases, if an exception is thrown the application is already broken and letting it limp along is an act of cruelty. The application should handle these critical failures as **_exceptional_** and handle all of these events the same way in an application scoped way (Custom Error Page for Web Applications, etc). By all means log the exception at the application scope level, it will have actual diagnostic value there (complete stack trace, etc).
