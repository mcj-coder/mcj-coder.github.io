...
The module gets the relevant configuration for the request and then checks every control adapter against the blacklist and whitelist, to determine if it’s allowed.  This is done using reflection to manipulate the ‘**_Adapter_**’ internal property( and it’s backing field **_\_adapter_**) of the System.Web.UI.Control type that every ASP.Net control is derived from.
